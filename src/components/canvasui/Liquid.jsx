...
export function createLiquid(elements, options = {}) {
  ...
  function createFluidTargets(simResolution, dyeResolution) {
    return {
  ...
  function rebuildFluidTargets(simResolution, dyeResolution) {
  ...
  // Reads the dye buffer back to the CPU so it can be reused outside this
  // WebGL context -- e.g. as a real distortion map on content rendered by
  // a *different* renderer (three.js, in this project's case), since two
  // separate WebGL contexts can't share a GPU texture directly. The dye
  // texture doubles as a displacement source already: applySplat() writes
  // the same (dx, dy) into both the velocity target and the dye target,
  // so dye.rg IS a (dissipating) displacement vector, not just colour --
  // exactly what FRAG_DISPLAY's own `fluid.rg * uDistortion` above reads.
  // Read as FLOAT (not the usual UNSIGNED_BYTE) because displacement is
  // signed and can exceed 1.0 -- an 8-bit readback would clamp away the
  // sign and the magnitude both. Resolution is small (dyeResolution) by
  // design, so this per-frame readback stays cheap.
  let dyeReadBuffer = null;
  function readDyeData() {
    const target = fluidTargets.dye.read;
    const size = target.width * target.height * 4;
    if (!dyeReadBuffer || dyeReadBuffer.length !== size) {
      dyeReadBuffer = new Float32Array(size);
    }
    gl.bindFramebuffer(gl.FRAMEBUFFER, target.fbo);
    try {
      gl.readPixels(
    ...
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    return { data: dyeReadBuffer, width: target.width, height: target.height };
  }
...
export function Liquid({
  ...
  className,
  style,
  // Not part of the fluid config -- a way for a parent to reach the
  // running instance (for its splat()/getDyeData()/etc), since this
  // component otherwise keeps that instance entirely to itself. Called
  // with the instance once it's created, and with null on unmount/before
